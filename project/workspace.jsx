...
const getInnerDims = (mode, view, frame) => {
  if (mode === 'mobile') {
    if (view === 'kanban') return { w: 1156, h: 1400 }; // 4 × 280 + 3 × 12 — wide enough to show ~1 full col + peek
...
// ─── Block ───────────────────────────────────────────────────
function Block({ block, pos, isPeeked, onClick }) {
  if (!pos) return null;
...
function BlockPeek({ block, pos, innerW, onClose }) {
  const POPOVER_W = 280;
  // Anchor below the block, or above if no space below. Default below.
  ...
  // Fake children so we have something to show
  const children = [
  ...
  return (
    <>
      {/* invisible scrim closes on outside click */}
      <div onClick={onClose}
...
function ViewSwitch({ view, onView, compact = false }) {
...
// ─── Workspace ──────────────────────────────────────────────
function Workspace({ mode = 'desktop', frame, view, onView, scrollableX = true }) {
...
Object.assign(window, { Block, Decoration, ViewSwitch, Workspace, VIEW_DEFS, getInnerDims });
